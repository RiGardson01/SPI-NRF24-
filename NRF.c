...
uint8_t TX_ADDRESS[TX_ADDR_WIDTH] = {0xEE,0xDD,0xCC,0xBB,0xAA};
uint8_t ACK[1] = {123};
...
void NRF24_Init (void)
{
	...
	NRF24_WriteReg(CONFIG, 0);//will config later
	NRF24_WriteReg(EN_AA, 0);//No auto Ack
	NRF24_WriteReg(EN_RXADDR, 0);//Not Enabling any data pipe right now
	NRF24_WriteReg(SETUP_AW, 0x03);//5 bytes for the TX or RX
	NRF24_WriteReg(SETUP_RETR, 0);//no retransmits
	NRF24_WriteReg(RF_CH, 0);//Will during TX or RX
...
void NRF24_Rx_ini(void)
{
	...
	NRF24_reset(0);
	NRF24_WriteReg(RF_CH, 23);
	NRF24_WriteReg(EN_RXADDR, 0x02);//select data pipe 1
	NRF24_WriteReg_Mutil(RX_ADDR_P1,TX_ADDRESS,TX_ADDR_WIDTH);
	NRF24_WriteReg(RX_PW_P1, TX_PLOAD_WIDTH);
	//power up the device
	config = NRF24L01P_Read_Register(CONFIG);
	config = config| (1<<1)|(1<<0);//PRIM_RX = 1:PRX
	Delay(2);
	NRF24_WriteReg (CONFIG, config);
...
{		
	...
	CS_ON();
	//SPI_Transmit(REG,1);//comand Read  RX FIFO 
	Delay(1);
...
{
	uint8_t status = NRF24L01P_Read_Register(STATUS);
...
void read_all_register (uint8_t *res)
{
	res[0]=NRF24L01P_Read_Register(CONFIG);
	res[1]=NRF24L01P_Read_Register(RF_CH);
	res[2]=NRF24L01P_Read_Register(EN_RXADDR);
	res[3]=NRF24L01P_Read_Register(FEATURE);
	res[4]=NRF24L01P_Read_Register(DYNPD);
	res[5]=NRF24L01P_Read_Register(SETUP_AW);	
}
